...
<main>
    ...
    $exibirResultados = mysqli_fetch_array($search);
      $id = $exibirResultados[0];
			...
			$debito = $exibirResultados[9];


		 echo "<div class='popupFixo'>
		     <div class='miniPopupContent'>
			    <h1 class='tituloBanner'>Cancelar Pagamento</h1><br><br>
					<h3 class='aviso'>Tem certeza que quer cancelar o pagamento da parcela $parcelaN/$parcelaT de $debito?</h3><br>

			 		<form class='form' method='post' action='includes/deletar_pagamento-inc.php?pay=$id'>

			      <input class='botao' name='submit' type='submit' value='Deletar'>
			 		</form>
						<a href='visualizar_cliente.php?cpf=$cpf'><img src='img/close.png' class='close'></a>
				</div>
			</div>
			<body>";
			?>
					...
					<table style='text-align:center'>
						<tr>
							...
							// PESQUISA VALOR EM ABERTO //
								 $cpf = filter_input(INPUT_GET, 'cpf', FILTER_SANITIZE_NUMBER_INT);
								 $termo_da_pesquisa = $_GET['cpf'];
								 $conteudo_pesquisa = "SELECT ParcelaN, ParcelaT, descricaoGeral, debito, credito, cliente, clienteCPF, dataDeb, dataCred, compraID, dataVenda, ID,
										                  	CASE
